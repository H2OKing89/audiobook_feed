...
if (process.env.NODE_ENV === 'production' && fs.existsSync(distPath)) {
  ...
  console.log('Serving static frontend files from dist directory');
} else {
  console.log('Development mode: frontend should be served separately on port 5006');
}
...
try:
    ...
    success = update_watchlist_entry_by_author(
        ...
        ${series_filter ? `"${series_filter.replace(/"/g, '\\"')}"` : 'None'},
        ${publisher_filter ? `"${publisher_filter.replace(/"/g, '\\"')}"` : 'None'},
        ${narrator_filter ? `"${narratorFilter.replace(/"/g, '\\"')}"` : 'None'}
    )
...
// Get all authors in watchlist
app.get('/api/watchlist', (req, res) => {
  const tempScriptPath = path.join(os.tmpdir(), `get_watchlist_${Date.now()}.py`);
  const scriptContent = `
import sys
import os
import json
...
try:
    ...
    cursor.execute("""
        SELECT author_name, title, series, publisher, narrator 
        FROM watchlist 
        WHERE active = 1 
        ORDER BY author_name, series
    """)
    ...
    for row in rows:
        ...
        # Add criteria if any fields are set
        if row['title'] or row['series'] or row['publisher'] or row['narrator']:
            authors[author]['criteria'].append({
                'title': row['title'] or '',
    ...
    # Handle authors with no criteria (add empty criteria array)
    for author in result:
        if 'criteria' not in author:
            author['criteria'] = []
...
`;
  ...
  pythonProcess.on('close', code => {
    ...
    if (code !== 0) {
      return res.status(500).json({ error: 'Failed to get watchlist', code });
    }
    
    try {
      // Parse only the last line as JSON in case there's other output
      const lines = dataString.trim().split('\n');
      const lastLine = lines[lines.length - 1];
      const result = JSON.parse(lastLine);
      res.json(result);
    } catch (e) {
      console.error('Error parsing watchlist result:', e);
      console.error('Raw output:', dataString);
      res.status(500).json({ 
        error: 'Failed to parse watchlist data', 
        details: dataString.substring(0, 1000)
      });
    }
  });
});

// Add new author to watchlist
...
try:
    db_path = 'audiobooks.db'
    author_name = """${name.replace(/"/g, '\\"')}"""
    criteria = json.loads('${JSON.stringify(criteria).replace(/'/g, "\\'")}')
    
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor();
    
    # Check if author already exists
    cursor.execute("SELECT 1 FROM watchlist WHERE author_name = ? LIMIT 1", (author_name,))
    if cursor.fetchone():
        print(json.dumps({"error": "Author already exists in watchlist"}))
        exit(1)
    
    # If no criteria provided, add author with empty criteria
    if not criteria:
        cursor.execute("""
            INSERT INTO watchlist (author_name, active, date_added) 
        ...
        for c in criteria:
            cursor.execute("""
                ...
                (author_name, title, series, publisher, narrator, active, date_added) 
                VALUES (?, ?, ?, ?, ?, 1, datetime('now'))
            """, (
                author_name, 
                c.get('title', ''), 
    ...
    conn.commit()
    print(json.dumps({"status": "success", "message": "Author added to watchlist"}))
...
`;
  ...
  pythonProcess.on('close', code => {
    ...
    if (code !== 0) {
      return res.status(500).json({ error: 'Failed to add author to watchlist', code });
    }
    ...
    try {
      ...
      console.error('Error parsing watchlist add result:', e);
      console.error('Raw output:', dataString);
      res.status(500).json({ 
...
// Update author in watchlist
app.put('/api/watchlist/:name', (req, res) => {
...
try:
    db_path = 'audiobooks.db'
    old_author_name = """${authorName.replace(/"/g, '\\"')}"""
    new_author_name = """${name.replace(/"/g, '\\"')}"""
    criteria = json.loads('${JSON.stringify(criteria).replace(/'/g, "\\'")}')
    
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor();
    
    # First deactivate all existing entries for this author
    cursor.execute("UPDATE watchlist SET active = 0 WHERE author_name = ?", (old_author_name,))
    ...
    if not criteria:
        ...
        # Add each criteria as new active entries
        for c in criteria:
            cursor.execute("""
                INSERT INTO watchlist 
                (author_name, title, series, publisher, narrator, active, date_added) 
    ...
    conn.commit()
    print(json.dumps({"status": "success", "message": "Author updated in watchlist"}))
...
`;
  ...
  pythonProcess.on('close', code => {
    ...
    if (code !== 0) {
      return res.status(500).json({ error: 'Failed to update author in watchlist', code });
    }
    ...
    try {
      ...
      console.error('Error parsing watchlist update result:', e);
      console.error('Raw output:', dataString);
      res.status(500).json({ 
        error: 'Failed to parse response', 
        details: dataString.substring(0, 1000)
...
try:
    ...
    if not cursor.fetchone():
        print(json.dumps({"error": "Author not found in watchlist"}))
        exit(1)
    
    # Delete all entries for this author
    cursor.execute("DELETE FROM watchlist WHERE author_name = ?", (author_name,))
    
    conn.commit()
    deleted_count = cursor.rowcount
    print(json.dumps({
        "status": "success", 
        "message": f"Author removed from watchlist", 
...
`;
  ...
  pythonProcess.on('close', code => {
    ...
    if (code !== 0) {
      return res.status(500).json({ error: 'Failed to delete author from watchlist', code });
    }
    ...
    try {
      ...
      console.error('Error parsing watchlist delete result:', e);
      console.error('Raw output:', dataString);
      res.status(500).json({ 
        error: 'Failed to parse response', 
        details: dataString.substring(0, 1000)
